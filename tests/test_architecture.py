...
def config(tmp_path: Path) -> dict:
    return {
        ...
        'domains': {'panel': 'panel.example.net', 'node': 'node.example.net', 'acme_email': 'a@example.net'},
        'cloudflare': {'api_token': 'token'},
        'hardening': {'enabled': False, 'ssh': {'enabled': False}, 'fail2ban': {'enabled': False}, 'unattended_upgrades': {'enabled': False}, 'system': {}},
    }
...
def test_task_names_are_unique() -> None:
    ...
    assert names[:3] == ['prerequisites', 'certificate', 'proxy-stack']
